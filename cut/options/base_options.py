...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--center_patch_offset', type=int, default=0, help='max random offset in pixels around the image center when using center_patch preprocess. With --center_patch_distribution gaussian this value is the standard deviation instead.')
        parser.add_argument('--center_patch_distribution', type=str, default='uniform', choices=['uniform', 'gaussian'], help='distribution of the random center offset for center_patch / random_center_patch. uniform: Uniform[-offset, offset]. gaussian: Normal(0, sigma=offset), clipped to image bounds.')
        parser.add_argument('--lambda_edge', type=float, default=0.0, help='weight for the mask-aware background edge loss: penalizes Sobel gradient in the generator output where the domain-A silhouette mask is 0 (background). Requires per-frame masks for domain A under <dataroot>/<phase>A_mask/. Assumes A is the structurally-known (e.g. simulation) domain; incompatible with --direction BtoA.')
